...
#define GET_DEFAULT_MEM_TYPE(mem_type)   {int current_device = -1; cudaGetDevice(&current_device); \
    struct cudaDeviceProp prop; cudaGetDeviceProperties(&prop, current_device); \
    if(prop.integrated) mem_type=NVBUF_MEM_SURFACE_ARRAY;\
    else mem_type=NVBUF_MEM_CUDA_DEVICE;}
